...
- **「プログラムから開く」＆関連付け起動**: エクスプローラーでPDFを右クリックして「プログラムから開く」や関連付け（ダブルクリック）で直接起動可能。複数ファイル選択時はファイルごとに個別ウィンドウ（別プロセス）が立ち上がります
...
- **手書きアノテーション**:
  ...
  - 筆圧トグル（スタイラスペンの筆圧による線の強弱・太さ変化を反映するトグルモード。デフォルトOFFで均一な線を描画。Windows Ink内蔵ペンに加え、自前WinTab連携によりWacomペンタブレット等のWindows Ink無効環境でも筆圧感知を完全サポート）
