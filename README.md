...
### 1. 俯瞰グリッド（バインダー整理）
- **タイル状サムネイル一覧**: 全ページを高精細サムネイル（最大360px対応・高DPI鮮明表示）で一目で確認
- **手書きメモのリアルタイム反映**: 詳細エディタで記入したペン・蛍光ペンのストロークがサムネイル一覧にも縮小合成されて即時反映
...
### 2. 詳細手書きエディタ
- **ズーム連動動的プレビュー**: 現在のズーム倍率や画面DPIに合わせてリアルタイム・デバウンス描画。縮小表示時も細線が消失せず文字がかすれず、拡大時もくっきり鮮明
- **ペンツール**: 自由な色・太さでの滑らかなベクター手書き
- **蛍光ペンツール**: 下の文字が透けて見える半透明ハイライト描画
- **消しゴムツール**: 一筆丸ごと消去する「ストローク消し」と、触れた部分だけ消す「部分消し」の切替
- **直線トグル**: ペンや蛍光ペンの色・太さを保ったまままっすぐ直線を引けるトグルモード（十字カーソルプレビュー付き）
