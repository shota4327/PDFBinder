...
### 1. 縦連続スクロールの詳細ビュー（基本画面）
- **快適な縦連続表示**: PDFを開くと全ページが縦一列に並ぶ連続ビューで即座に閲覧・編集開始
...
- **表示オプション（詳細ビュー）**:
  ...
  - ウィンドウサイズ変更時やカレントページ変更時にも最適倍率を自動維持するモード追従型（手動ズーム操作時にカスタム固定倍率へシームレスに移行）
- **拡大縮小の一本化**: 詳細ビュー（ページ表示倍率）とグリッドビュー（サムネイルサイズ）のズーム操作を一本化。縮小、等倍/標準リセット、拡大を直感的に操作可能
