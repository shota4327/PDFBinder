...
- **統合タイトルバー兼タブバー（Chrome / Edge スタイル）**:
  ...
  - **タブ配置**: 「PDF編集」「手書き」タブ。選択中のタブは下部の白ツールバー領域とシームレスに結合。
...
- **リボンタブ構成**:
  - **「PDF編集」タブ**: グリッド俯瞰時の標準タブ（常時利用可能）
    ...
    - 表示ズーム: 縮小 (`Ctrl+-`), 拡大 (`Ctrl++`)（140px〜360px、1クリック20px増減、上限・下限でのボタン非活性制御）
  - **「手書き」タブ**: ページ詳細エディタ表示時のみ選択可能（編集開始で自動選択、一覧復帰で自動解除）
    - 戻る: 一覧に戻る
    - ツール: 選択 (`Cursor`), ペン (`Pen`), 蛍光ペン (`Highlighter`), 直線 (`Line`), 全体消し (`EraserStroke`), 部分消し (`EraserPoint`), 移動 (`Pan`)
    - 太さ & 色:
      - 太さプリセット: 0.5px, 1.0px, 2.0px, 4.0px のドット（●）選択ボタン
      - カラーパレット: 5色プリセット（黒 `#000000`、鮮やかな赤 `#EF4444`、鮮やかな青 `#2563EB`、鮮やかな緑 `#16A34A`、鮮やかな黄 `#EAB308`）
...
### 6.4 詳細手書きエディタビュー
- **画面構成**:
  - ツールバーは上部リボンの「手書き」タブに完全統合され、エディタ画面側は広大なキャンバス領域をフル表示
- **キャンバスエリア**:
  - 背景にPDFiumのズーム連動動的レンダリング結果（現在のズーム倍率および画面DPIに合わせた最適解像度でのラスタライズにより、縮小時の細線消失・文字かすれや拡大時のぼやけを解消）
  - デバウンス制御（150ms）、ダブルバッファリング、世代番号管理により、高速操作時のチラつき（フリッカー）や非同期競合を完全に防止
  - 前面にWPF `InkCanvas`（`EditorInkCanvas`）をオーバーレイ配置（論理座標系の維持によりインクのズレなし）
