...
1. **通常起動**:
   - 引数なしで起動された場合、ドキュメント未読み込み状態（0件）でメインウィンドウを表示する。
...
2. **「プログラムから開く」および関連付け起動**:
   ...
   - 複数ファイルが渡された場合は、同一ウィンドウ内で全ファイルを別ドキュメント（セッション）として順次開き、タイトルバーのプルダウンから自由に切り替えられるようにする。
   - 存在しないファイルや無効な引数が渡された場合は、エラー通知を表示し、アプリ自体は安全に起動を継続する。
3. **ウィンドウ状態・サイズの復元**:
...
「PDF編集」タブの「印刷」ボタンまたはショートカットキー `Ctrl+P` により、左右2分割レイアウトのモダンなインアプリ・オーバーレイ（幅880px × 高さ600px）を表示します。
- **左側：印刷設定パネル**:
  - **プリンター選択**: システムにインストールされている利用可能なプリンター一覧から選択（初期値: OSの既定プリンター）。
  - **部数設定**: 1〜99部（数値入力およびステッパーボタンによる増減、初期値: 1）。
  - **用紙の向き**: 「縦」「横」の切り替え（初期値: 縦）。
  - **用紙サイズ**: 「A4」「A3」の切り替え（初期値: A4）。
  - **両面印刷**: 「片面」「両面（長辺とじ）」「両面（短辺とじ）」の3択（初期値: 片面）。
  - **印刷範囲**: 「すべてのページ（初期値）」「現在のページのみ」「ページ指定（カスタム）」の3択。カスタム入力時は「1-3, 5, 8」などの指定をリアルタイム検証し、無効な数値や範囲外指定を赤枠・エラーメッセージで通知し「印刷」ボタンを無効化。
  - **印刷モード**:
    - 「用紙サイズに合わせる（Fit to Page）」: 1枚の用紙に1ページをアスペクト比維持で最大化配置（初期値）。
    - 「１ページに集約（N-up）」: 1枚の用紙に 2 / 4 / 8 ページをグリッド状に縮小配置して印刷。
    - 「冊子形式（製本）」: 中綴じ二つ折り用の面付けを自動計算。用紙サイズが4の倍数でない場合は末尾に白紙を自動補完し、用紙の向きを「横」、両面設定を「両面（短辺とじ）」に自動連動。
- **右側：用紙プレビュー領域**:
  - 用紙の縦横比・余白・面付け結果（単一、N-up、冊子）をリアルタイムに高品位レンダリングして自動Fit表示。手書きストロークが存在する場合は忠実に合成描画。
