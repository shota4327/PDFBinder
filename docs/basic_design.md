...
- **リボンタブ構成**:
  ...
  - **「手書き」タブ**: ページ詳細エディタ表示時のみ選択可能（編集開始で自動選択、一覧復帰で自動解除）
    - 戻る: 一覧に戻る
    - ツール: 選択 (`Cursor`), ペン (`Pen`), 蛍光ペン (`Highlighter`), 直線 (`Line`), 全消しゴム (`EraserStroke`), 部分消し (`EraserPoint`), 手のひら (`Pan`)
    - 太さ & 色:
      - 太さプリセット: 0.5px, 1.0px, 2.0px, 4.0px のドット（●）選択ボタン
      - 自由選択: トグルボタンでインラインスライダー（0.5px〜24.0px）を展開・微調整
      - カラーパレット: プリセットカラーサークル一覧
...
- **キャンバスエリア**:
  ...
  - 前面にWPF `InkCanvas`（`EditorInkCanvas`）をオーバーレイ配置
  - **パームリジェクション & マルチタッチジェスチャー**:
    ...
    - マウス操作: ツールバーの「✋ 手のひら」選択でドラッグスクロール、ペン選択で描画、ホイールでスクロールを維持
