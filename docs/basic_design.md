...
### 6.2 グリッド俯瞰ビュー（メイン画面）
- **ツールバー**:
  - ファイル操作（リボンスタイル）: 開く (`Ctrl+O`), 追加, 保存 (`Ctrl+S`), 別名保存 (`Ctrl+Shift+S`)
  - ページ構成・抽出（リボンスタイル）: 白紙追加 (`Ctrl+B`), 選択抽出, 全分割
  - ページ編集（リボンスタイル）: 左回転 (`Ctrl+L`), 右回転 (`Ctrl+R`), 削除 (`Delete` / ホバー時赤ハイライト)
  - 履歴操作（リボンスタイル）: 元に戻す (`Ctrl+Z`), やり直す (`Ctrl+Y`)
  - 表示ズーム（リボンスタイル）: 縮小 (`Ctrl+-`), 拡大 (`Ctrl++`)（140px〜360px、1クリック20px増減、上限・下限でのボタン非活性制御）
- **サムネイルグリッド**:
