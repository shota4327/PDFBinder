...
コンテンツ表示領域の下端に重ねて配置する半透明オーバーレイ方式（上部境界線 `#CBD5E1`、半透明白 `#D9FFFFFF`（不透明度85%））を採用し、背後のコンテンツを上品に透過表示。高さを約5px拡大（MinHeight 42px、Padding 12,5）し、各コントロールのサイズ（ボタン 28x28px、入力欄 26px）を拡大して視認性とクリックしやすさを向上。単一ページ表示（FitToWindow）時はステータスバー上部に収まるようフィット領域を補正しスクロールバー誤出現を防止。連続表示およびグリッド表示でも最下部スクロール時に最後のページやサムネイルカードがステータスバーに隠れないよう、下部オフセット（62px）を確保。
