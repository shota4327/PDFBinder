...
- **アプリアイコンクリック操作**:
  ...
  - メニュー項目「version」（アイコンなし、14ptテキスト）を選択することで、バージョン情報モーダルダイアログを起動。
- **バージョン情報モーダルダイアログ（`AboutDialog`）**:
  - アプリのダークテーマ（`#1E1E1E`, `#252526`, `#3E3E42`）に調和した角丸（Radius: 10px）および滑らかなドロップシャドウ付きの専用モーダルウィンドウ。
  - **表示要素**:
    - アプリアイコン（64×64 px）
    - アプリ名（`PDF Binder`、22pt 太字）
    - バージョンバッジ（アセンブリから動的抽出されたセマンティックバージョン、14pt、例: `v0.1.0`）
    ...
    - プライマリスタイルの「閉じる」ボタン（`Esc` キー、Enter キー、外枠ドラッグ移動に対応）
