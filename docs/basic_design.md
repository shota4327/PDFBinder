...
「PDF編集」タブの「印刷」ボタン（アイコン: `print` `&#xE8AD;`）またはショートカットキー `Ctrl+P` により、表示領域の左右幅いっぱいに連動拡大（高さ750px、左右マージン24px）するモダンな左右2分割インアプリ・オーバーレイを表示します。
- **左側：印刷設定パネル（幅360px）**:
  - **プリンター選択**: タイトルバーのファイル選択と同様の美しいモダンComboxBox（`ModernDarkComboBoxStyle`）により、選択中のプリンター名が明瞭に表示されるドロップダウン形式（初期値: OSの既定プリンター）。
  - **部数設定**: ウィンドウ右下のズームコントロールと同様の一体型デザイン（`[-]` `[ 1 ]` `[+]`、角丸4px、14px大きめ文字）。
  - **用紙の向き**: `[ 縦 ]` `[ 横 ]` のボタントグル（Segmented Control / 初期値: 縦）。
  - **用紙サイズ**: `[ A4 ]` `[ A3 ]` のボタントグル（初期値: A4）。
  - **両面印刷**: `[ 片面 ]` `[ 両面(長辺) ]` `[ 両面(短辺) ]` の横並びボタントグル（初期値: 片面）。
  - **印刷範囲**: `[ すべて ]` `[ 現在のページ ]` `[ ページ指定 ]` のボタントグル。ページ指定選択時は直下の入力欄が活性化し、「1-3, 5, 8」などの指定をリアルタイム検証。無効時は赤枠・エラー通知し「印刷」ボタンを無効化。
  - **印刷モード**:
    - `[ サイズに合わせる ]` `[ 1Pに集約 ]` `[ 冊子(製本) ]` のボタントグル。
    - **1Pに集約**: 選択時、直下のモダンComboxBoxで集約ページ数（2 / 4 / 8ページ）を選択可能。
