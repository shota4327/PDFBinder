...
- **キャンバスエリア**:
  ...
  - **拡大縮小連動ペンホバープレビューカーソル (WYSIWYG)**:
    ...
    - 通常ペンは選択色の塗りつぶし円、蛍光ペンは半透明の塗りつぶし円、部分消しゴムは中心ドットのない消去範囲輪郭線リングを、8x8スーパーサンプリングおよび乗算済みアルファ（Premultiplied Alpha）による滑らかなアンチエイリアス処理を施した 32-bit ARGB DIB カーソルとしてメモリ内キャッシュで高速生成。
