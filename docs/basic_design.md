...
- **左側：印刷設定パネル（幅360px、縦スクロール対応）**:
  - **プリンター選択**: タイトルバーのファイル選択と同様のフラット・ボーダレスなモダンComboxBox（`ModernDarkComboBoxStyle`）により、選択中の文字のすぐ右横に下三角アイコンが寄り添うデザイン（初期値: OSの既定プリンター）。
  - **部数設定**: ウィンドウ右下のズームコントロールと同様の一体型デザイン（`[-]` `[ 1 ]` `[+]`、角丸4px、15pt大きめ文字）。
  - **用紙の向き**: `[ 縦 ]` `[ 横 ]` の左詰めボタントグル（Segmented Control / 初期値: 縦）。
  - **用紙サイズ**: `[ A4 ]` `[ A3 ]` の左詰めボタントグル（初期値: A4）。
  ...
  - **印刷モード**:
    ...
    - **1Pに集約**: 選択時のみ直下に集約数ドロップダウン（2 / 4 / 8ページ）が展開。
