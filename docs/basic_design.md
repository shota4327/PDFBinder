...
### 6.2 グリッド俯瞰ビュー（メイン画面）
- **ツールバー**:
  - ファイル: 開く (`Ctrl+O`), 上書き保存 (`Ctrl+S`), 名前を付けて保存 (`Ctrl+Shift+S`)
  - ページ操作: 左90度回転 (`Ctrl+L`), 右90度回転 (`Ctrl+R`), 削除 (`Delete`), 白紙追加 (`Ctrl+B`)
  - ファイル連携: PDF追加結合, 選択ページ抽出（分割）, 全ページ個別分割
  - 履歴: 元に戻す (`Ctrl+Z`), やり直す (`Ctrl+Y`)
  - 表示: サムネイルサイズ拡大・縮小スライダー
- **サムネイルグリッド**:
