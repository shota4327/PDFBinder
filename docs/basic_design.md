...
- **リボンタブ構成**:
  ...
  - **「手書き」タブ**: ページ詳細エディタ表示時のみ選択可能（編集開始で自動選択、一覧復帰で自動解除）
    - 戻る: 一覧に戻る
    - ツール: 選択 (`Cursor`), ペン (`Pen`), 蛍光ペン (`Highlighter`), 直線 (`Line`), 全体消し (`EraserStroke`), 部分消し (`EraserPoint`), 手のひら (`Pan`)
    - 太さ & 色:
      - 太さプリセット: 0.5px, 1.0px, 2.0px, 4.0px のドット（●）選択ボタン
      - カラーパレット: 4色プリセット（黒 `#000000`、鮮やかな赤 `#EF4444`、鮮やかな青 `#2563EB`、鮮やかな緑 `#16A34A`）
