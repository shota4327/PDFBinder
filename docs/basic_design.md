...
コンテンツ表示領域の下端に重ねて配置する半透明オーバーレイ方式（上部境界線 `#CBD5E1`、半透明白 `#BFFFFFFF`（不透明度75%））を採用し、背後のコンテンツを上品に透過表示。高さを約5px拡大（MinHeight 42px、Padding 12,5）し、各コントロールのサイズ（ボタン 28x28px、入力欄 26px）を拡大して視認性とクリックしやすさを向上。単一ページ表示（FitToWindow）時はページがステータスバーと重ならないよう下部オフセット（42px）およびフィット有効領域の補正を行い、スクロールバーを誤出現させずにステータスバー上部に綺麗に配置。
- **配置**: コンテンツ領域の最前面下部にオーバーレイ配置（`Panel.ZIndex="10"`、単一ページ時は下部オフセットにより重複を回避）
- **左端（ページナビゲーション）**:
  - 書式: `< ページ [ 1 ] / 2 >`
