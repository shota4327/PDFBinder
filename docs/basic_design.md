...
3. **変更履歴管理（`CHANGELOG.md`）**:
   - [Keep a Changelog 1.1.0](https://keepachangelog.com/ja/1.1.0/) に準拠し、最上部に `[Unreleased]` セクションを保持する。
   - 各Issue/PR作業時に、加えた変更内容を該当カテゴリ（`### Added`, `### Changed`, `### Fixed` 等）へ追記する。
4. **自律的インクリメント＆リリースフロー（パターン2）**:
   - PRマージ完了時に、該当PRの変更内容に応じてエージェントが自律的にバージョンをインクリメント（新機能: マイナー `0.X.0` / 不具合修正・改善: パッチ `0.X.Y`）する。
   - `CHANGELOG.md` の `[Unreleased]` を新バージョンとして確定し、Gitタグ（`vX.Y.Z`）の作成および `build.ps1` による最新配布バイナリの自動発行を実施する。
