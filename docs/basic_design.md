...
PDFBinder/
├── .gitignore
├── Directory.Build.props         # バージョン情報・共通ビルドプロパティの一元定義（SemVer正本）
├── CHANGELOG.md                  # 変更履歴（Keep a Changelog 1.1.0 準拠）
...
- `DocumentKind`: `DocumentKind` (ドキュメント種別: `Pdf`, `Image`)
- `IsImage`: `bool` (画像ドキュメントフラグ)
- `Pages`: `ObservableCollection<PdfPageModel>` (ページ順序リスト)
- `IsModified`: `bool` (未保存変更の有無)
- `CanUndo` / `CanRedo`: `bool` (アンドゥ・リドゥ可能状態)

### 4.3 `DocumentSession`
1つの開いているPDFまたは画像ドキュメントの作業状態（データ、履歴、表示コンテキスト）をカプセル化するモデル。
- `Document`: `PdfDocumentModel` (ドキュメントデータ)
- `IsImage`: `bool` (画像ドキュメントフラグ)
...
### 5.8 `IImageService`
画像ファイル（JPEG / PNG）の読み込み、回転・手書きストロークの合成保存、およびPDF書き出しを担当。
- `bool IsSupportedImage(string? filePath)`: 対象ファイルがサポート対象の画像形式（.jpg, .jpeg, .png）か判定
...
- **カスタムスクロールバー＆浮遊オーバーレイ配置**:
  - Windows標準の淡色スクロールバーを廃止し、チャコールダーク背景に馴染むスリムでモダンなカスタムスクロールバースタイル（幅・高さ12px、角丸4px、矢印ボタン排除、ホバー・ドラッグ対応）を全体適用しています。
  - 下部ステータスバーとの重複を回避するため、スクロールビューアーには浮遊オーバーレイテンプレート（`OverlayScrollViewerStyle`）を採用。用紙コンテンツは最下部（ステータスバー背後）まで自然にスクロール可能としつつ、横スクロールバーおよび縦スクロールバーをステータスバー直上（42px上）に浮遊配置することで、操作部が隠れる問題を根本から解消しています。
  - 詳細ビューおよびグリッドビューの双方で一貫して適用され、詳細ビューでは `Shift + マウスホイール` による直感的な水平スクロールにも対応しています。
...
- **統合タイトルバー兼タブバー（Chrome / Edge スタイル）**:
  ...
  - **タブ配置**: 「編集」「手書き」「表示」タブ。選択中のタブは下部の白ツールバー領域とシームレスに結合。
...
- **リボンタブ構成**:
  - **「編集」タブ**: ファイル操作およびドキュメント構成の編集（常時利用可能）
    ...
    - ページ編集: 左回転 (`Ctrl+L`), 右回転 (`Ctrl+R`), 削除 (`Delete` / ホバー時赤ハイライト)
      - ※詳細ビュー表示時、特定ページが明示選択されていない場合は現在操作・表示中のカレントページを対象として回転・削除・白紙挿入が実行される。白紙追加時は追加された新しい白紙ページへ自動的に表示・スクロールが遷移し、Undo時には追加元のページが復元・表示される。
    - 履歴操作: 元に戻す (`Ctrl+Z`), やり直す (`Ctrl+Y`)
    - **画像ドキュメント編集時のUI制御（機能グレーアウト）**:
      - 画像ファイル（JPEG / PNG）の単一編集時は、ドキュメントの構成変更や複数ページ操作に関わる機能が無効化（グレーアウト）される:
        - 無効化: 「追加」（結合）、「白紙追加」、「選択抽出」、「全分割」、「ページ分割」、「削除」
        - 有効（利用可能）: 「開く」、「保存」（元画像形式で上書き保存）、「別名保存」（JPEG/PNG/PDF書き出し）、「印刷」、「左回転」、「右回転」、「元に戻す」、「やり直す」
  - **「手書き」タブ**: 詳細ビュー表示時のみ選択可能（グリッドビュー時は非活性化、グリッド切替時は他タブへ自動遷移。画像編集時もPDFと同様に完全利用可能）
    ...
    - テキスト選択・リンク操作（インタラクティブオーバーレイ）:
      - 文字選択ツール時: PDF内の文字をマウスドラッグで自由に矩形・範囲選択でき、半透明ブルーでハイライト表示。`Ctrl+C` や右クリックコンテキストメニュー「コピー」によりクリップボードへテキストをコピー可能（画像ドキュメント時はテキストが存在しないため空動作）。
  ...
  - **「表示」タブ**: ビュー切り替え、表示オプション、ページ配置およびズーム操作の一本化
    - ビュー切り替え: 「詳細」ラジオボタン（手書き・閲覧詳細表示）、「グリッド」ラジオボタン（サムネイル一覧表示。※画像ドキュメント編集時は単一画像編集に特化するためグリッド切替は無効化・グレーアウト）
    - 表示オプション（詳細ビュー時のみ有効、ラジオボタングループ）:
      - 「100%」: 等倍（1.0倍）表示（画像の場合は元画像のピクセル解像度1:1表示）
    ...
    - ページ配置（詳細ビュー時のみ有効、ラジオボタングループ）:
      ...
      - 「連続」: 全ページを縦に並べて連続スクロール表示（※画像ドキュメント編集時は単一画像であるため無効化・グレーアウト）。
    - ズーム操作: 縮小 (`Ctrl+-`), 倍率表示/リセット (`Ctrl+0`), 拡大 (`Ctrl++`), マウスホイールズーム (`Ctrl + Wheel`)
      - 詳細ビュー時: ページ表示倍率（5%〜3200%）を操作、適応型スナップ目盛り（5%〜3200%の全28段階）により高倍率から極小縮小まで直感的に拡大縮小、クリックで等倍（100%）にリセット。大判図面PDFや高解像度写真画像でも画面にぴったりフィット可能。単一・連続表示ともに `Ctrl + Wheel` による拡大縮小に対応
      - グリッドビュー時: 適応型スナップ目盛り（50%〜3200%、基準サイズ220pxに対して110px〜7,040px）により段階的に拡大縮小（サムネイル極小化防止のため下限50%を維持保護）、クリックで等倍（100% / 220px）にリセット。`Ctrl + Wheel` による直感的な拡大縮小に対応。サムネイル事前生成解像度を倍増（720x1008）し高倍率でも鮮明に表示
...
- **中央（ステータス表示）**:
  - 現在の操作状況やファイル読み込み完了メッセージを動的表示（左端のページ表示と重複するページ番号文字列は排除し、エラー発生時は例外詳細の直接露出を避けて「保存に失敗しました。」等の簡潔なステータスを表示）
- **右端（プログレス＆ズームコントロール）**:
  - 非同期処理中のプログレスバー
  - ズームコントロール: `[-] 100% [+]`
    - 縮小ボタン (`-`): 拡大率を1段階縮小（表示タブの縮小コマンドと連動）
    - 倍率表示・表示モード切り替えボタン: 現在の拡大率（%）を表示。詳細ビュー時はクリックするたびに「100%表示」→「ウィンドウに合わせる」→「幅に合わせる」を順繰りにサイクル切り替え（手動ズーム等からのクリック時は「ウィンドウに合わせる」に切り替え）。グリッドビュー時はクリックでサムネイル倍率を標準サイズ（100%）にリセット
...
- **サムネイルグリッド**:
  ...
  - **ドラッグ中スクロール＆オートスクロール（操作性向上）**:
    - ページドラッグ（並び替え・外部PDF挿入）中およびラバーバンド矩形選択中に、マウスホイールによる縦スクロールが可能。ドラッグ中は誤操作防止のためズーム操作を自動抑止し、スクロールに追従して挿入バーや選択範囲をリアルタイム更新
    - グリッド表示領域の上下端（40px領域）にカーソルが進入した際、端に近づくほど速度が上がる可変速度オートスクロールを実行
...
- **キャンバスエリア**:
  ...
  - **ストローク・ビットマップキャッシュ層 & 描画パイプライン最適化**:
    ...
    - **ストローク重なり順（Z-Order）保持と蛍光ペン重ね塗り濃色化**: 蛍光ペンストロークは `IsHighlighter = false` かつ半透明色（アルファ値 120 / 約47%）として管理・描画。ペン文字の上に蛍光ペンを引いた場合は書いた順番通りに文字の上に色が重なり、蛍光ペン同士を重ね塗りした場合は色が自然に濃くなる表現を実現。エディタ、ストロークキャッシュ、サムネイル合成、PDF出力の全系統で完全な順序整合性を維持
...
### 6.9 アプリアイコン・プルダウンおよびバージョン情報（About画面）
- **アプリアイコンクリック操作**:
  - タイトルバー最上段左端のアプリアイコン（`Assets/icon.png`）をクリック可能（`WindowChrome.IsHitTestVisibleInChrome="True"`）とし、クリック時に下方向にダークテーマのプルダウンメニュー（`ContextMenu`）を展開。
  - メニュー項目「version」（アイコンなし、14ptテキスト）を選択することで、バージョン情報ダイアログをインアプリ・オーバーレイとして起動。
- **バージョン情報インアプリ・オーバーレイ（`AboutOverlayControl`）**:
  - メインウィンドウ前面に暗転レイヤー（Scrim: `ModalBackdropGridStyle`）と角丸12pxのFluentカード枠（`ModalCardBorderStyle`）を配置したインアプリ・オーバーレイ形式。
  - **表示要素**:
    - アプリアイコン（64×64 px）
    - アプリ名（`PDF Binder`、22pt 太字）
    - バージョンバッジ（アセンブリから動的抽出されたセマンティックバージョン、14pt、例: `v0.2.0`）
    - アプリ概要コピー（「直感的で軽快なWindows向けPDFバインダー＆手書き編集ツール」）
    - ランタイム情報（`.NET 10.0 (win-x64)`）
    - ライセンス情報（寛容型オープンソース: MIT / Apache-2.0 / BSD）
    - 著作権表記（`Copyright © 2026 PDF Binder Contributors`）
    - GitHubリポジトリリンク（クリック時に既定ブラウザで起動）
    - プライマリスタイルの「閉じる」ボタン（クリック、`Esc` キー、およびカード外側の暗転背景クリックによるスムーズなクローズに対応）

### 6.10 モーダルダイアログ・インアプリ・オーバーレイ共通UI仕様
アプリケーション内で使用されるモーダル表示（未保存変更確認、印刷設定・プレビュー、バージョン情報、エラー・警告通知）は、`App.xaml` で定義された共通スタイルによりデザインの完全な一貫性を保持します。
- **暗転背景スタイル (`ModalBackdropGridStyle`)**:
  - 半透明黒背景（`#80000000`）、フォーカス不可（`Focusable="False"`）。
- **カード枠スタイル (`ModalCardBorderStyle`)**:
  - サーフェス背景（`{StaticResource SurfaceBackgroundBrush}`）、境界線（`{StaticResource SurfaceBorderBrush}`、幅1px）、角丸（`CornerRadius="12"`）、滑らかなドロップシャドウ（BlurRadius: 28px, Opacity: 0.5, ShadowDepth: 8px）。
- **インタラクションの責務分離**:
  - 未保存変更確認ダイアログおよび印刷ダイアログは、ユーザーの重要判断や作業中の誤操作を防ぐためカード外側の暗転背景クリックでは閉じず、ボタン操作またはEscキーによる明示的アクションを必須とする。
  - エラー・警告通知ダイアログ（`IsErrorDialogVisible`）は、ファイル読み込み、保存、結合、分割などの例外発生時、およびページ未選択でのエクスポートなどの操作警告時に画面前面へ表示され、重要操作の失敗や誤操作を確実にユーザーへ伝達する。ボタン操作（OK）、Enterキー、またはEscキーでのみ閉じ、カード外側の暗転背景クリックでは閉じない安全設計とする。
  - バージョン情報ダイアログは情報閲覧目的であるため、ボタン・Escキーに加えて暗転背景クリックでも軽快に閉じられる親切設計とする。
...
## 8. 画像ファイル（JPEG / PNG）の直接編集・合成保存仕様

PDF Binder は、PDFファイルに加えて一般的な画像形式（JPEG: `.jpg`, `.jpeg` / PNG: `.png`）を直接開き、回転および手書きアノテーションを加えて元画像形式またはPDFとして保存するワークフローを完全サポートします。

### 8.1 ドキュメント・セッション管理
1. **独立ドキュメント化**:
   - 画像ファイルを開いた際、単一ページ（1ページ）の `PdfDocumentModel`（`DocumentKind = DocumentKind.Image`）として独立したタブ（新規セッション）を生成します。
   - 複数画像ファイルをまとめて開いた場合でも、1つのドキュメントに結合せず、必ず画像1枚ごとに独立した新規タブとして展開します。
2. **ドラッグ＆ドロップ動作**:
   - グリッド俯瞰ビューまたは詳細ビューへの画像ファイルドロップ時は、既存ドキュメントへのページ挿入を行わず、常に独立した新規タブとしてオープンします。
   - 画像ドキュメント編集中に外部PDFファイルがドロップされた場合も、同様に新規PDFタブとしてオープンします。
3. **ビュー制御**:
   - 画像ドキュメントは単一画像に特化するため、初期表示は「詳細ビュー（単一ページ）」とし、グリッドビューへの切り替えおよび縦連続スクロール表示は無効化（グレーアウト）します。

### 8.2 解像度・DPI整合性と表示倍率
1. **DPIメタデータ認識と物理寸法算出**:
   - 元画像のピクセル寸法（`PixelWidth`, `PixelHeight`）およびDPI情報（`DpiX`, `DpiY`）を取得・保持します。
   - スキャン書類等で明示的な高DPIメタデータ（120 DPI以上）が存在する場合、`Width = PixelWidth * 72.0 / DpiX`、`Height = PixelHeight * 72.0 / DpiY` として物理寸法（pt）を算出し、300 DPI スキャン書類などを自動的に標準A4サイズ（595×842pt）等として自然に展開します。
   - 一般的な写真やスクリーンショット（96 DPI以下または未指定）の場合は、従来のピクセル等倍原則（`PixelWidth * 72.0 / 96.0`）を維持し、等倍（100%）表示時に 1:1 ピクセル（原寸）でくっきりと表示されます。
2. **ズーム範囲拡張による大解像度画像の自動フィット**:
   - 最小ズーム倍率を `0.05`（5%）まで拡張し、4000×3000pxなどの巨大な高解像度写真画像であっても「ウィンドウにあわせる」「幅にあわせる」で画面内に完璧に収まります。
3. **大解像度画像における手書きストローク太さ・カーソルの自動適正化**:
   - キャンバス寸法が巨大な画像ドキュメント（`DocumentKind == DocumentKind.Image`）において、基準A4短辺（595.0 pt）に対する画像の最小辺の比率（`scale = Math.Max(1.0, minDim / 595.0)`）を算出。
   - ペン、蛍光ペン、消しゴムの太さ、およびホバープレビューカーソルにこのスケーリング係数を連動適用することで、画面全体を見ながら筆記した際に「通常のA4用紙に書いているときと全く同じ視覚的太さ・書き心地」を常に保証します。
   - ※PDFドキュメントの場合はスケーリング係数を常に `1.0`（無補正）とし、既存のPDFにおける手書き太さや表示を 100% そのまま維持します。
4. **画像表示レンダリング**:
   - `PdfiumRenderer` において、画像ファイルの場合はPDFiumのラスタライズ処理をバイパスし、メモリ上に展開された元画像ビットマップをそのまま背景画像として返却します。
   - テキスト選択用の文字抽出や注釈リンク抽出はスキップ（空データを返却）し、オーバーヘッドをゼロに抑えます。

### 8.3 画像合成保存およびPDF書き出しパイプライン
1. **元画像形式での上書き保存 (`Ctrl+S`)**:
   - 元の画像形式（JPEG または PNG）を判別し、同一フォーマットで出力します。
   - **回転処理**: ページモデルの回転（`Rotation`）に応じた `RotateTransform` を適用し、元画像のピクセル配列を劣化なく物理回転します。
   - **手書きストローク合成**:
     - 元画像のピクセル解像度（96 DPI基準）に合わせるため、`targetPixelWidth / page.DisplayWidth`（= 96.0 / 72.0）のスケール変換を適用した `DrawingVisual` に `InkStrokes.Draw(dc)` を実行します。
     - `RenderTargetBitmap` により元画像の物理ピクセル解像度と同一寸法の高解像度ストローク画像をラスタライズし、元画像の上にアルファブレンディング合成します。
     - PNG保存時は可逆圧縮（ロスレス）、JPEG保存時は高品質（品質レベル約92%）エンコーダーを使用してファイル書き出しを行います。
     - 保存時は一時ファイル（`.tmp`）へ安全に出力した後に不可分置換（アトミックな置き換え）を実施し、ファイル破損を防止します。
2. **「名前を付けて保存」およびPDF書き出し**:
   - 保存ダイアログにおいて、`JPEG画像 (*.jpg;*.jpeg)`, `PNG画像 (*.png)`, `PDFドキュメント (*.pdf)` のフィルタを選択可能とします。
   - PDF形式を選択した場合は、`PdfSharp` を用いて画像の回転後の寸法に合致する単一ページPDFを新規生成し、回転済み元画像を描画した上で、アピアランス付き手書き注釈（`/AP /N`）およびISF可逆メタデータ（`/PdfBinderInk`）を埋め込んで保存します。
...
2. **堅牢性**:
   - 不正なPDF、破損ページ、パスワード保護PDF、無効な画像ファイルに対する適切なエラーハンドリングとダイアログ通知。
3. **テスト自動化**:
   - `PDFBinder.Core` 内のページ操作、マージ、スプリット、白紙挿入、画像読み込み・合成保存の単体テスト網羅率100%（全テストPASS必須）。

---

## 10. バージョン管理およびリリース運用仕様

1. **セマンティックバージョニング（SemVer 2.0.0）の徹底**:
   - バージョン番号は `Major.Minor.Patch`（例: `0.1.0`）形式を採用する。
   - 初期バージョンは `0.1.0` とし、正式版 `1.0.0` への繰り上げはユーザーからの明示的な指示によって行う。
2. **正本一元管理（`Directory.Build.props`）**:
   - リポジトリルートに配置された `Directory.Build.props` を正本（Single Source of Truth）とし、全サブプロジェクトおよび出力EXEバイナリへ自動伝播させる。
3. **変更履歴管理（`CHANGELOG.md`）**:
   - [Keep a Changelog 1.1.0](https://keepachangelog.com/ja/1.1.0/) に準拠し、最上部に空の `[Unreleased]` セクションを保持する。
   - 各Issue/PRの作業ブランチにおいて、新バージョン（`## [X.Y.Z] - YYYY-MM-DD`）を確定・記述し、PRの差分で変更履歴をレビュー可能とする。
4. **自律的インクリメント＆リリースフロー（方式①）**:
   - 各Issue/PRの作業ブランチ内で、該当PRの変更内容に応じてエージェントが自律的にバージョンをインクリメント（新機能: マイナー `0.X.0` / 不具合修正・改善・ドキュメント: パッチ `0.X.Y`）し、`CHANGELOG.md` を確定する。
   - PRマージ後、`master` 上ではコミットを一切行わず、最新化されたマージコミットに対して直ちにGitタグ（`vX.Y.Z`）の作成および `build.ps1` による最新配布バイナリの自動発行を実施する。
