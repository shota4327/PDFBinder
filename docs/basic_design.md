...
- **外部ファイルドラッグ＆ドロップ（詳細ビューと共通）**:
  - ドキュメント読み込み済みの状態で外部PDFファイルをドラッグすると、メイン表示領域全体に半透明のドロップ案内オーバーレイ（「ここにPDFをドロップして開く」）を表示
  - ドロップされたPDFを別ドキュメント（新しいタブ）として順次オープン（未読み込み時も同様にオープン。現在のドキュメントへのページ結合はツールバーの「追加」ボタンより実行）
- **サムネイルグリッド**:
