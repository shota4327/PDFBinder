...
- **リボンタブ構成**:
  ...
  - **「手書き」タブ**: 詳細ビュー表示時のみ選択可能（グリッドビュー時は非活性化、グリッド切替時は他タブへ自動遷移）
    ...
    - 描画オプション（直線・筆圧・太さ・色）:
      ...
      - 筆圧トグル: 直線ボタンの右横に配置。ペン使用時かつ直線モード無効時のみ操作可能なトグルボタン（初期値OFF）。OFF時は線の太さを均一（`IgnorePressure = true`）にし、ON時はスタイラスペンの筆圧に応じた強弱・太さ変化（`IgnorePressure = false`）を反映。Windows Inkネイティブ対応ペン（Lenovo等の内蔵デジタイザ）に加え、自前P/InvokeによるWinTab API（`wintab32.dll`）連携を搭載し、Wacomペンタブレット等の「Windows Ink を使う」が無効化された環境でも筆圧感知描画を完全サポート。タブレット検出時はステータスバーへの通知およびツールチップへのデバイス名表示（例: `WinTab: Wacom Intuos 検出済み`）を行い、WinTab未導入環境でもクラッシュゼロで安全にフォールバック。ツール切り替え時や別PDF読み込み時も設定状態を保持。
