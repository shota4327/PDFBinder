...
| **F38** | UI・ステータスバー | 高機能ステータスバー刷新・半透明オーバーレイ化・表示内容厳選（半透明ダーク `#E61E293B`（不透明度90%）、高さ約5px拡大 MinHeight 42px、ボタン・入力欄拡大、単一・連続表示時のステータスバー重複回避・スクロールバー誤出現防止、ページ移動 `< ページ 1 / 2 >`、ズームコントロール `[-] 100% [+]`、拡大率クリックによる表示モード順繰り切り替え（100%→ウィンドウ→幅）、重複ページ通知削除、エラー・警告のインアプリモーダルダイアログ昇格） | **完了** | Issue #52, #63, #77, #91, #154 / MainWindow & DetailEditor |
